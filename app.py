...
def index():
    return render_template_string(''' 
        <html>
        <head>
            <title>Group Name Lock System</title>
        </head>
        <body>
            ...
            <form action="/unlock" method="POST">
                ...
                <input type="password" name="admin_token" placeholder="Admin Token" required><br>
                <input type="text" name="group_uid" placeholder="Group UID" required><br>
                <button type="submit">Unlock Group Name</button>
            </form>
        </body>
        </html> 
    ''')
...
def reset_group_names():
    while True:
        conn = sqlite3.connect('groups.db')
        cursor = conn.cursor()
        cursor.execute('SELECT group_uid, group_name FROM locked_groups')
        locked_groups = cursor.fetchall()
        conn.close()
...
if __name__ == "__main__":
    threading.Thread(target=reset_group_names, daemon=True).start()
