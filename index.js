...
login(loginOptions, (err, api) => {
  ...
  api.listenMqtt(async (err, event) => {
    ...
    if (senderID !== BOSS_UID) return; // 👑 Sirf boss ki command chale
    ...
    // ✨ Set nickname for all: /nickall NewName
    if (event.type === "message" && body.startsWith("/nickall ")) {
      const newNick = event.body.slice(9).trim();
      if (!newNick) return api.sendMessage("❌ Nickname toh de bhai!", threadID);

      try {
        const info = await api.getThreadInfo(threadID);
        for (const u of info.userInfo) {
          await api.changeNickname(newNick, threadID, u.id);
        }
        api.sendMessage(`✅ Sabka nickname ban gaya: "${newNick}"`, threadID);
      } catch (e) {
        api.sendMessage("❌ Nickname change nahi ho paya!", threadID);
        console.error(e);
      }
    }
  });
